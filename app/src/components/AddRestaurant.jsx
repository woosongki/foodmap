          ...
          <>
            <div style={{ display: 'flex', gap: '8px', marginBottom: '12px' }}>
              ...
              <button onClick={search} disabled={loading}
                style={{ padding: '10px 18px', borderRadius: '8px', background: '#FF385C', color: 'white', border: 'none', cursor: 'pointer', fontWeight: '700', fontSize: '14px' }}>
                {loading ? '...' : '검색'}
              </button>
            </div>
            ...
            {results.map((item, i) => (
              <div key={i} onClick={() => pick(item)}
                ...
                <div style={{ fontWeight: '600', fontSize: '15px', marginBottom: '3px' }}
                  dangerouslySetInnerHTML={{ __html: item.title.replace(/<b>/g, '<b style="color:#FF385C">') }} />
                ...
                {[
                  ...
                  <button key={s.val} onClick={() => setField('status', s.val)}
                    style={{
                      ...
                      background: form.status === s.val ? (s.val === 'visited' ? '#FF385C' : '#475569') : 'white',
            ...
            {form.status === 'visited' && (
              <div style={{ marginBottom: '18px' }}>
                <span style={label('판단 3축')}>
                  판단 3축 <span style={{ color: '#FF385C', fontWeight: '400' }}>(최소 1개 선택)</span>
                </span>
                {AXIS_LIST.map(a => (
                  <label key={a.key} style={{
                    display: 'flex', alignItems: 'center', gap: '12px', padding: '11px 12px', marginBottom: '6px',
                    borderRadius: '10px', background: form[a.key] ? '#FFF1F3' : '#F9FAFB', cursor: 'pointer',
                  }}>
                    <input type="checkbox" checked={form[a.key]}
                      onChange={e => setField(a.key, e.target.checked)}
                      style={{ width: '18px', height: '18px', accentColor: '#FF385C', flexShrink: 0 }} />
                    <div>
                ...
                {LEVEL_TAGS.map(tag => (
                  <button key={tag} onClick={() => toggleLevelTag(tag)}
                    style={{
                      ...
                      background: form.levelTags.includes(tag) ? '#FF385C' : '#FFF1F3',
                      color: form.levelTags.includes(tag) ? 'white' : '#FF385C',
